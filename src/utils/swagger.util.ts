...
class SwaggerController extends BaseController {
    ...
    const response = await this.get('https://petstore3.swagger.io/api/v3/openapi.json', {
        headers: {
    ...
    writeFileSync(swaggerJSONPath, JSON.stringify(response.body, null, 2), 'utf-8');
...
export async function setupSwagger() {
    ...
    console.log('🏗️ Generating types for Pet Store APIs from the Swagger Schema');
    execSync(`npx openapi-typescript "${swaggerJSONPath}" --output "${swaggerTypesPath}"`, { stdio: 'inherit'});
    console.log(`✅ Swagger Types generated successfully in the path: '${swaggerTypesPath}'`);
    } catch (error) {
        throw new Error('Failed to process swagger generation of Pet Store')
    }
}